GibsonOS.define('GibsonOS.module.ftp.index.listener.itemDblClick', function(view, record, item, index, event, options) {
    var store = view.getStore();
    var proxy = store.getProxy();
    var dir = proxy.getReader().jsonData.dir;

    if (record.get('type') == 'dir') {
        GibsonOS.module.ftp.index.fn.open(store, dir + record.get('name') + '/');
    } else {
        ...
        if (localPath) {
            GibsonOS.module.ftp.index.fn.download(dir, record.get('name'), localPath, {
