Ext.define('GibsonOS.module.ftp.index.Tree', {
    extend: 'GibsonOS.tree.Panel',
    alias: ['widget.gosModuleFtpIndexTree'],
    itemId: 'ftpIndexTree',
    requiredPermission: {
        module: 'ftp',
        task: 'index'
    ...
    itemContextMenu: [{
        ...
        xtype: 'gosModuleFtpIndexDownloadButton',
        handler: function() {
            ...
            var localPath = null;
            var activeTab = GibsonOS.module.ftp.index.fn.getActiveNeighborTab(parent);
            ...
            if (localPath) {
                GibsonOS.module.ftp.index.fn.download(record.get('id'), null, localPath, {
        ...
        handler: function() {
            ...
            GibsonOS.module.ftp.index.fn.delete(dir, [], {
                id: extraParams.id ? extraParams.id : null,
                url: extraParams.url ? extraParams.url : null,
                port: extraParams.port ? extraParams.port : null,
                protocol: extraParams.protocol ? extraParams.protocol : null,
    ...
    initComponent: function() {
        ...
        me.store = new GibsonOS.module.ftp.index.store.Tree({
            gos: {
                data: {
                    tree: this
                    /*extraParams: {
        ...
        me.on('cellkeydown', function(view, td, cellIndex, record, tr, rowIndex, event) {
            if (event.getKey() == Ext.EventObject.DELETE) {
                ...
                GibsonOS.module.ftp.index.fn.delete(record.get('id'), [], {
                    id: extraParams.id ? extraParams.id : null,
                    url: extraParams.url ? extraParams.url : null,
                    port: extraParams.port ? extraParams.port : null,
                    protocol: extraParams.protocol ? extraParams.protocol : null,
